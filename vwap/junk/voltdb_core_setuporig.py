...
def wait_for_statefulset_object_to_exist(statefulset_name, namespace, timeout_seconds=120):
    ...
    while time.time() - start_time < timeout_seconds:
        ...
        if result.stdout.strip() == f"statefulset.apps/{statefulset_name}": # Corrected object name format
...
def main():
    ...
    try:
        # CORRECTED: Removed capture_output=True for helm repo update when stdout is DEVNULL
    ...
    docker_secret_name = "voltdb-docker-registry-secret" # Default secret name for VoltDB Core
    ...
    if install_new_cluster:
        ...
        if create_secret.lower() == "yes":
            if check_kubernetes_secret_exists(docker_secret_name, volt_ns):
                ...
                docker_server = get_user_input("Enter Docker Registry Server (e.g., docker.io)", default="docker.io")
    ...
    # Add a small sleep here to allow Kubernetes API to settle after object exists
    time.sleep(5) # Added sleep to address potential race condition
